...
interface AudioPlayerFixedProps {
  title: string
  speaker: string
  duration: string | null  // ALLOW NULL HERE
  audioUrl: string
}
...
export default function AudioPlayerFixed({ 
  ...
  // Format time - handle null duration
  ...
  const getDisplayDuration = () => {
    ...
    if (duration.includes(':')) {
      ...
      if (parts.length === 3) {
        // Convert HH:MM:SS to total seconds for display
    ...
    <div className="bg-gray-900 text-white rounded-xl p-6 shadow-lg">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-gray-400">
          {currentTime} / {getDisplayDuration()}
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-800 text-xs text-gray-500">
        <div className="flex justify-between">
          <span>File: {audioUrl.split('/').pop()}</span>
          <span>Speed: {playbackRate}x • Vol: {Math.round(volume * 100)}%</span>
        </div>
      </div>
    </div>
  )
